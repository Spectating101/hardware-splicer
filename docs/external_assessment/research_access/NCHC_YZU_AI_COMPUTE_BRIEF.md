...
Hardware-Splicer's next evidence tranche includes repeated live external/open-model evaluation against a frozen adversarial corpus. If YZU has or obtains NCHC access, local/open-model runs could support:
...
Do not claim H200 compute is required for API-only OpenAI/Anthropic evaluations. The NCHC case is strongest when tied to **actual local/open-model inference or training/evaluation workloads**.
...
Do not guess a large number. Estimate from measured or vendor/model runtime.

For inference:

`models × cases × repeats × average runtime per case / GPU concurrency`
...
For any fine-tuning/training, create a separate budget based on the actual method and dataset size.
