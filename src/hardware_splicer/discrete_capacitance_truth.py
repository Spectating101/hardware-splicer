...
def audit_ldo_capacitance_evidence(proposal: Mapping[str, Any], evidence: Mapping[str, Any]) -> Dict[str, Any]:
    ...
    if ldo is None:
        return {"status":"failed","hard_failures":[{"code":"LDO_EVIDENCE_MISSING"}],"unresolved":[],"authority_effect":"none","power_on_authorized":False}
    ...
    for passive in list(proposal.get("passives") or []):
        ...
        if rail_pair == {"+5V", "GND"}:
            role, nominal_min, operating_bias_v = "ldo_input", minimum_nominal_in, 5.0
        elif rail_pair == {"+3V3", "GND"}:
            role, nominal_min, operating_bias_v = "ldo_output", minimum_nominal_out, 3.3
        if role is None:
            continue
        ...
        row = {
            "ref": ref, "role": role, "nominal_capacitance_uf": nominal,
            "minimum_nominal_uf": nominal_min, "minimum_effective_uf": minimum_effective,
            "operating_bias_v": operating_bias_v, "mpn": exact_mpn,
            "effective_capacitance_uf_at_operating_bias": effective_value,
        }
        if nominal < float(nominal_min or 0.0):
            row["status"] = "fail"
            hard_failures.append({"code":"NOMINAL_CAPACITANCE_BELOW_DATASHEET_MINIMUM","ref":ref,"role":role,"nominal_uf":nominal,"minimum_uf":nominal_min})
        elif exact_mpn is None or effective_value is None:
            row["status"] = "unresolved"
            unresolved.append({"code":"EFFECTIVE_CAPACITANCE_UNRESOLVED","ref":ref,"role":role,"reason":"Nominal capacitance is present, but exact capacitor identity and/or effective capacitance at operating DC bias is not persisted.","minimum_effective_uf":minimum_effective})
        elif effective_value < minimum_effective:
            row["status"] = "fail"
            hard_failures.append({"code":"EFFECTIVE_CAPACITANCE_BELOW_STABILITY_MINIMUM","ref":ref,"role":role,"effective_uf":effective_value,"minimum_effective_uf":minimum_effective})
        else:
            row["status"] = "closed"
        rows.append(row)
    ...
    for required_role in ("ldo_input", "ldo_output"):
        if required_role not in roles:
            hard_failures.append({"code":"REQUIRED_LDO_CAPACITOR_MISSING","role":required_role})
