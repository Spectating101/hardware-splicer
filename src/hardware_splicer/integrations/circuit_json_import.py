...
def circuit_json_to_netlist(
    ...
    *,
    ...
    for doc in source_ports:
        ...
        pin = PinRef(component_ref=ref, pin=str(pin_value))
        port_id_to_pin[port_id] = pin
        port_metadata[port_id] = {
            "component_ref": ref,
            "pin": pin.pin,
            "name": doc.get("name"),
            "port_hints": list(doc.get("port_hints") or []),
        }
    ...
    # Backwards compatibility for the earliest Hardware-Splicer adapter, where
    # schematic_trace.source_net_id was used as the net name and ports were
    # serialized directly as REF.PIN strings.
