"""Local token ledger for text LLM calls (Qwen, agy/Gemini, etc.)."""
...
LEDGER_SCHEMA_VERSION = "hardware_splicer.text_usage_ledger.v1"
DEFAULT_LEDGER_PATH = Path("data/llm/hardware-splicer-text-usage.json")
...
def ledger_path(configured: Optional[str] = None) -> Path:
    raw = str(configured or os.getenv("HARDWARE_SPLICER_TEXT_LEDGER") or DEFAULT_LEDGER_PATH).strip()
    return Path(raw)
...
def record_text_usage(
    *,
    ...
    entry = {
        ...
        "cached": bool(cached),
        "prompt_excerpt": str(prompt_excerpt or "")[:160],
    ...
    data["entries"] = entries[-5000:]
    data["updated_at"] = entry["ts"]
    target.write_text(json.dumps(data, indent=2) + "\n", encoding="utf-8")
    return entry


def usage_summary(*, path: Optional[Path] = None, provider: Optional[str] = None) -> Dict[str, Any]:
    target = path or ledger_path()
    data = _load_ledger(target)
    entries = _list_dicts(data.get("entries"))
    if provider:
        entries = [row for row in entries if str(row.get("provider") or "") == provider]
    ...
    for row in entries:
        ...
        if ts.startswith(day_prefix):
            day_tokens += tokens
        for bucket_key, bucket in ((model, by_model), (prov, by_provider), (stage, by_stage)):
            slot = bucket.setdefault(
                bucket_key,
                {"calls": 0, "cached_calls": 0, "total_tokens": 0},
            )
            slot["calls"] += 1
            if row.get("cached"):
                slot["cached_calls"] += 1
    ...
    return {
        "schema_version": "hardware_splicer.text_usage_summary.v1",
...
def _free_tier_estimate(model: str, consumed_tokens: int) -> Dict[str, Any]:
    env_key = f"QWEN_FREE_TIER_TOTAL_{model.upper().replace('-', '_').replace('.', '_')}"
    configured = os.getenv(env_key)
    env_total = os.getenv("QWEN_TEXT_FREE_TIER_TOTAL")
