...
export default function VisualEngineeringWorkbenchPage() {
  ...
  async function loadProject(selectedProjectId: string) {
    ...
    try {
      ...
      setSelectedObject(nextGraph.objects[0] || null);
    } catch (caught) {
  ...
  async function loadProjects() {
    ...
    try {
      ...
      setProjects(listed);
      const first = projectId || text(listed[0]?.project_id, '');
      ...
      <div className="mx-auto max-w-[2200px]">
          ...
          <div className="flex flex-col gap-4 xl:flex-row xl:items-center xl:justify-between">
            <div className="flex items-center gap-4">
              ...
              <div>
                ...
                <p className="mt-1 max-w-4xl text-sm leading-6 text-slate-400">Public renderers handle file-specific drawing. Hardware Splicer keeps object identity, evidence, failures, proposals, revisions, JARVIS context, and authority coherent across them.</p>
              </div>
            </div>
        ...
        <section className="mt-3 overflow-hidden rounded-[1.6rem] border border-white/10 bg-white/5">
          <div className="flex min-h-14 items-center gap-2 overflow-x-auto border-b border-white/10 bg-[#06101d] px-3 py-2">
            {engineeringVisualAdapters.map((adapter) => {
          ...
          <div className="grid min-h-[calc(100vh-11rem)] gap-px bg-white/10 xl:grid-cols-[260px_minmax(0,1fr)_370px]">
            <aside className="bg-[#06101d] p-4">
              <div className="flex items-center justify-between gap-3">
                <div>
                  ...
                  <div className="mt-1 text-sm font-semibold text-white">Object tree</div>
                </div>
                  ...
                  return (
                    ...
                    >
                      <div className="flex items-center justify-between gap-2">
                        ...
                        {object.status === 'blocked' ? <ShieldAlert className="h-3.5 w-3.5 text-rose-300" /> : object.status === 'supported' ? <CheckCircle2 className="h-3.5 w-3.5 text-emerald-300" /> : <Eye className="h-3.5 w-3.5 text-violet-300" />}
                      </div>
              ...
              <div className="mt-5 rounded-2xl border border-white/8 bg-[#020711] p-3">
                <div className="text-[10px] font-semibold uppercase tracking-[0.16em] text-slate-500">Layers</div>
                    ...
                    <div className="mt-6 grid gap-4 md:grid-cols-2">
                      <div className="rounded-2xl border border-white/10 bg-[#020711] p-4">
                        ...
                        <div className="mt-3 space-y-2 text-xs text-slate-300">
                          ...
                          <div className="flex justify-between gap-3"><span className="text-slate-500">Selected object</span><span className="max-w-[12rem] truncate">{selectedInCurrentGraph?.label || 'none'}</span></div>
                        </div>
                      </div>
                      <div className="rounded-2xl border border-white/10 bg-[#020711] p-4">
              ...
              {selectedInCurrentGraph ? (
                <div className="mt-5 space-y-4">
                  ...
                  <section className="rounded-2xl border border-white/10 bg-[#020711] p-4">
                    ...
                    <div className="mt-3 space-y-2">
                      {physicalGates.map(([label, value]) => <div key={String(label)} className="flex items-center justify-between rounded-xl border border-white/8 px-3 py-2 text-[10px]"><span className="text-slate-400">{String(label)}</span><span className={value === true ? 'text-rose-200' : 'text-emerald-200'}>{value === true ? 'authorized' : 'closed'}</span></div>)}
                    </div>
                  </section>
