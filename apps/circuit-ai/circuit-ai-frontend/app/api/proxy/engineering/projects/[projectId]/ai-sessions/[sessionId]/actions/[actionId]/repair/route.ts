...
export async function POST(request: Request, context: RouteContext) {
  ...
  const target = `${getHardwareSplicerApiUrl()}/v1/projects/${encodeURIComponent(projectId)}/ai-sessions/${encodeURIComponent(sessionId)}/actions/${encodeURIComponent(actionId)}/repair`;
