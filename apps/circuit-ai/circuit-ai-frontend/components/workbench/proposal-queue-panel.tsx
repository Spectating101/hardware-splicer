    ...
    <aside className="flex h-full min-h-0 flex-col border-l border-white/10 bg-[#07101d]">
      <div className="border-b border-white/10 p-3">
        <div className="flex items-center gap-2">
          ...
          <span className="text-[10px] font-semibold uppercase tracking-[0.18em] text-cyan-300">Proposal queue</span>
        </div>
        <div className="mt-2 text-xs font-semibold text-white">{candidate?.name}</div>
        <p className="mt-1 text-[10px] leading-4 text-slate-500">{candidate?.note}</p>
        <div className="mt-2 rounded-lg border border-amber-300/10 bg-amber-300/[0.035] px-2.5 py-2 text-[10px] leading-4 text-amber-100/65">
              ...
              <div key={proposal.id} className={`rounded-xl border p-3 transition ${selected ? 'border-cyan-300/25 bg-cyan-300/[0.045]' : 'border-white/8 bg-white/[0.02]'}`}>
                <button type="button" aria-label={`Inspect proposal ${proposal.title}`} onClick={() => inspectProposal(proposal)} className="w-full text-left">
                  <div className="flex items-start gap-2">
                    <span className={`rounded border px-1.5 py-0.5 text-[8px] font-semibold tracking-[0.12em] ${opTone(proposal.operation)}`}>{proposal.operation}</span>
                    <div className="min-w-0 flex-1">
                      <div className="text-[10px] font-medium text-slate-100">{proposal.title}</div>
                      {resource ? <div className="mt-0.5 text-[9px] uppercase tracking-[0.12em] text-slate-600">{resource.kind} · {resource.name}</div> : null}
                    </div>
                    <Crosshair className="mt-0.5 h-3 w-3 shrink-0 text-slate-600" />
                  </div>
                  <p className="mt-2 text-[10px] leading-4 text-slate-500">{proposal.rationale}</p>
                  <div className="mt-2 flex gap-2 rounded-md border border-white/6 bg-black/10 px-2 py-1.5 text-[9px] leading-3 text-slate-600">
                ...
                <div className="mt-2 flex items-center gap-1.5 border-t border-white/7 pt-2">
                  <span className="mr-auto text-[9px] uppercase tracking-[0.12em] text-slate-600">{effectiveState}</span>
                  {proposal.state !== 'rejected' ? (
                    <>
                      <button type="button" aria-label={`Hold proposal ${proposal.title}`} onClick={() => setProposalDecision(proposal.id, 'held')} className={`inline-flex items-center gap-1 rounded-md border px-2 py-1 text-[9px] font-medium ${decision === 'held' ? 'border-amber-300/25 bg-amber-300/8 text-amber-200' : 'border-white/8 text-slate-500 hover:text-amber-200'}`}>
                        <CirclePause className="h-3 w-3" /> Hold
                      </button>
                      <button type="button" aria-label={`Accept proposal ${proposal.title} to working candidate`} onClick={() => setProposalDecision(proposal.id, 'accepted')} className={`inline-flex items-center gap-1 rounded-md border px-2 py-1 text-[9px] font-medium ${decision === 'accepted' ? 'border-emerald-300/25 bg-emerald-300/8 text-emerald-200' : 'border-white/8 text-slate-500 hover:text-emerald-200'}`}>
