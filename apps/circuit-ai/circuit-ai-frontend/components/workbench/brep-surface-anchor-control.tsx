...
import { Crosshair, Loader2, MousePointer2, X } from 'lucide-react';
...
export function BrepSurfaceAnchorControl({
  ...
  const pickStatus = useWorkbenchBrepAnchorStore((state) => state.pickStatus);
  const pickMessage = useWorkbenchBrepAnchorStore((state) => state.pickMessage);
  ...
  const feedbackApplies = Boolean(
    armed
    || (pickStatus !== 'idle' && Object.values(anchors).some((anchor) => anchor.entityId === entityId)),
  );
