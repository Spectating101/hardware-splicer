...
test('constructor consumes live resource_strategy.v1 projections without weakening authority', async ({ page }) => {
  ...
  await page.route('**/api/proxy/resource/strategy', async (route) => {
    ...
    await route.fulfill({
      ...
      body: JSON.stringify(strategyResponse(mode)),
    });
  });
  ...
  await expect(page.getByText('Hybrid strategy covers the target by retaining evidenced donor islands and procuring the power gap.', { exact: true })).toBeVisible();
  ...
  await expect(page.getByText('Open procurement covers the target with documented modules and fewer evidence gaps.', { exact: true })).toBeVisible();
