#!/usr/bin/env python3
"""Audit one imported Winbond W25Q128JW-Q Verilog capture without executing it."""
...
def main() -> int:
    ...
    result = audit_winbond_verilog_capabilities(args.archive.read_bytes(), manifest)
    rendered = json.dumps(result, indent=2, sort_keys=True) + "\n"
    if args.out:
        args.out.parent.mkdir(parents=True, exist_ok=True)
        args.out.write_text(rendered, encoding="utf-8")
    print(rendered, end="")
    return 0 if result["jedec_id_read_9f_source_candidate"] else 2
