#!/usr/bin/env python3
"""Validate or run the fresh unseen SPI-flash cleanroom corpus."""
...
from hardware_splicer.cleanroom_unseen_spi_flash_experiment import (  # noqa: E402
    run_unseen_spi_flash_live_experiment,
...
def _summary(result: Mapping[str, Any]) -> str:
    lines = [f"mode={result.get('mode') or 'unknown'}"]
    if result.get("mode") == "unseen_spi_flash_corpus_validation":
        lines.extend(
            [
                f"pass={bool(result.get('pass'))}",
                f"case_count={int(result.get('case_count') or 0)}",
                f"equivalent_case_count={int(result.get('equivalent_case_count') or 0)}",
...
def main() -> int:
    ...
    parser.add_argument("--out-dir", default="artifacts/unseen-spi-flash")
    ...
    if args.mode == "validate":
        result = validate_unseen_spi_flash_corpus()
        report_path = out_dir / "UNSEEN_SPI_FLASH_CORPUS.json"
        summary_path = out_dir / "CORPUS_SUMMARY.txt"
    ...
    result = run_unseen_spi_flash_live_experiment(model=args.model)
    report_path = out_dir / "LIVE_UNSEEN_SPI_FLASH_EXPERIMENT.json"
    summary_path = out_dir / "LIVE_UNSEEN_SUMMARY.txt"
    _write(report_path, result)
    summary_path.write_text(_summary(result), encoding="utf-8")
    print(summary_path.read_text(encoding="utf-8"), end="")
