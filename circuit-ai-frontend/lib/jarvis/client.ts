// Server-side Anthropic client. Uses fetch against the REST API — no SDK
// dependency. Designed to run inside Next.js route handlers.
//
// Models:
//   - claude-sonnet-4-6     → chat + identify + salvage + project (primary)
//   - claude-haiku-4-5      → cheap-fast cache-miss fallback when available
//
// Falls back to echo-mode when ANTHROPIC_API_KEY is unset so devs can exercise
// the UI wiring without spending tokens.
...
export async function callJarvis(opts: JarvisCallOptions): Promise<JarvisResult> {
  if (!hasApiKey()) {
    if (hasMistralKey() && (opts.flow === "chat" || opts.flow === "project")) {
